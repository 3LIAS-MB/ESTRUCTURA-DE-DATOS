...
public class BinarySearchTree<ELEMENT extends Comparable<ELEMENT>> extends BinaryTree<ELEMENT> {
    ...
    // generar y devolver una lista ordenada de elementos a partir de 
    // los elementos almacenados en el árbol binario de búsqueda.
    public DoubleLinkedOrderedList<ELEMENT> generarLista(){
    	DoubleLinkedOrderedList<ELEMENT> aux=new DoubleLinkedOrderedList<ELEMENT>();
    	return generarLista(this.root, aux);
    }
    // Realiza un recorrido inorden del árbol binario de búsqueda.
    protected DoubleLinkedOrderedList<ELEMENT> generarLista(BTNode<ELEMENT> root, DoubleLinkedOrderedList<ELEMENT> aux){
    	if (root != null) {
            aux=generarLista(root.left,aux);
            aux.addInOrder(root.item);
            aux=generarLista(root.right,aux);		
    	}
        return aux;
    }
