...
public class Restaurante {
    ...
    private QueueMejorado<Pedido> pedidosEnEspera;  // -> numeroPedido, orden, numeroMesa, estado	  // Esto especifica el tipo de la variable. En este caso, es un tipo genérico 	
    private QueueMejorado<Pedido> pedidosEnPreparacion;												  // llamado QueueMejorado, que es una implementación personalizada o extendida 
    private Stack<Boleta> pagosRecibidos; 															  // de una cola, y se espera que contenga objetos de tipo Pedido.
    
    //El <Boleta> especifica el tipo de datos que se almacena en la instancia de Stack
    //que QueueMejorado es una instancia específica de una clase genérica que maneja objetos de ese tipo.
 ...
 // Método para verificar si un código ya ha sido utilizado
    ...
    public void altaPedido() {
        ...
        //System.out.print("Ingrese el número de la mesa para realizar el pedido: ");
        	...
        	if(mesa.getNumeroMesa() == numMesa) {
        		if (mesa.getNumeroMesa() == numMesa) {
                    ...
                    for (Plato plato : platos) {
                        System.out.println(plato.getCodigo() + ". " + plato.getDescripcion() + " - $" + plato.getPrecioUnitario());
                    }

                    int[] orden = new int[4]; // guarda el pedido del usuario
                    for (int i = 0; i < 4; i++) {
                        System.out.print("Seleccione el plato ingresando el codigo (0 para finalizar) - " + (i+1) + ": ");
                    ...
                    Pedido pedido = Pedido.crearPedido(orden, numMesa); // -> retorna una instancia de 'Pedido', asigna el orden
    ...
    public void prepararPedidos() {
        ...
        for (int i = 0; i < 5 && !pedidosEnEspera.isEmpty(); i++) {
        	contador += 1;
            Pedido pedido = pedidosEnEspera.remove(); // estado, numeroMesa. numero -> pedidosEnEspera no remueve, solo le resta 1 al cointador
    ...
    public void pagarConsumos() {
        ...
        for (Mesa mesa : mesas) {
            if (mesa.getEstado().equals("ocupada") && mesa.getServicio().equals("servida")) { // si el estado de la mesa es ocupada y el servicio es servida
